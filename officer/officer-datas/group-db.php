...
// Get group By $program, $part, $season... For add student groups
function getAllGroupByPPS($program, $part, $season, $conn) {
    $sql = "SELECT * FROM groups WHERE program=? AND part=? AND season=?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$program, $part, $season]);
...
// Get group By $program, $part, $season... For add timetable
